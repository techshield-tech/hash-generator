...
```bash
git clone https://github.com/techshield-tech/hash-generator.git
cd hash-generator
bun install
bun dev
```

Open the URL Vite prints — by default **http://localhost:5173/hash-generator/**
(note the `/hash-generator/` path, see [Base path](#base-path)).
...
| otherwise (default)     | `/hash-generator/` | GitHub Pages                |
...
1. Go to [vercel.com/new](https://vercel.com/new) and import the
   `techshield-tech/hash-generator` repository.
...
`bun run build` with the default `/hash-generator/` base, and publishes `dist/`.
...
<iframe
  id="hash-generator"
  src="https://techshield-tech.github.io/hash-generator/?embed=1&theme=dark"
  style="width: 100%; border: 0;"
  title="JSON Formatter"
></iframe>

<script>
  const iframe = document.getElementById('hash-generator');

  window.addEventListener('message', (event) => {
    const data = event.data;
    if (data?.slug !== 'hash-generator') return;
...
| iframe → parent | `{ type: 'mmoall-tool:ready', slug: 'hash-generator' }`          | Posted once on mount (embed mode only) |
| iframe → parent | `{ type: 'mmoall-tool:height', slug: 'hash-generator', height }` | Posted whenever the document height changes (embed mode only) |
